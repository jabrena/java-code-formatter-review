...
public class Problem20211227 {
    ...
    public static void main(String[] args) {
        ...
        Function<Integer, BigDecimal> calculateIteration = (n) -> {
            ...
            return BigDecimal.valueOf(4 * n).divide(BigDecimal.valueOf(3).pow(n - 2, precision), roundingMode);
        };

        var iterations = 100;

        var result = IntStream.range(1, iterations).boxed().map(calculateIteration).reduce(BigDecimal.ZERO,
                BigDecimal::add); // Sum
